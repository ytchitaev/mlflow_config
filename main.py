...
from functions.model_runner import instantiate_model, split_dataset, perform_grid_search
...
# Parse CLI args
'''
python main.py -c config_iris.json
'''
    ...
    with mlflow.start_run() as run:
        ...
        try:
            ...
            logger, file_handler_path = instantiate_python_logger(
                experiment_run_path, configs['run_temp_folder'], configs['python_logging_file_name'])
            logger.info("Running mlflow...")
            logger.info(f"mlruns/{{experiment}}/{{run}}/ path: {experiment_run_path}")
            ...
            X_input, y_input = load_data('sklearn', dataset_name='iris')

            # Split data
            logger.info("Splitting data...")
            X_train, X_validation, X_test, y_train, y_validation, y_test = split_dataset(
                X_input, y_input, configs['train_percentage'], configs['validation_percentage'], configs['test_percentage'])

            # Instantiate model
            logger.info("Loading initial model...")
            model = instantiate_model(configs['library_name'], configs['model_name'])
            ...
            best_model = instantiate_model(configs['library_name'], configs['model_name'], best_params)
            best_model.fit(
                X_train,
                y_train,
            ...
            mlflow_log_artifact_cv_results(
                experiment_id, run_id, configs['run_temp_folder'], configs['cv_results_file_name'], cv_results)
