...
def get_full_path(path_values: Union[str, List[Union[str, int]]], file_name: Union[str, int] = None):
    ...
    path_values = [str(path_values)] if isinstance(path_values, (str, int)) else list(map(str, path_values))
...
def setup_run(run):
    ...
    artifacts_path = get_full_path([MLRUNS_DIR, experiment_id, run_id, ARTIFACTS_DIR])
    model_path = get_full_path([MLRUNS_DIR, experiment_id, run_id, MODEL_SUBDIR])
...
def output_last_exec_json(run_id, experiment_id, experiment_run_path, artifacts_path, model_path):
    last_exec_path = get_full_path(OUTPUTS_DIR, LAST_EXEC_FILE_NAME)
    last_exec_data = define_last_exec(run_id, experiment_id, experiment_run_path, artifacts_path, model_path)
