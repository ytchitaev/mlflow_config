...
def main(cfg: dict, ext: str, debug: bool, last_exec: dict):
    ...
    if ('plot_best_estimator_evals_result' in ext) and ('best_estimator_evals_result' in last_exec['artifacts_list']):
        best_estimator_evals_result = load_json(get_relative_path(
    ...
    if ('plot_cv_results' in ext) and ('cv_results' in last_exec['artifacts_list']):
        cv_results = load_csv(get_relative_path(
...
if __name__ == "__main__":
    ...
    last_exec = load_json(get_relative_path(get_config(config_global, 'global.outputs_dir'), get_config(config_global, 'global.last_exec_file_name')))
